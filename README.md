...
- クリックでポップオーバー: 全ウィンドウのゲージ、**リセット予定時刻(絶対時刻+残り時間)**、プラン表示、各種設定(サービスON/OFF・表示形式・更新間隔・**しきい値通知(80/90/100%)**・**ログイン時に起動**)。
...
作業に没頭している間も、画面の隅で**残量とリセットまでの時間がずっと見えている**——
「あと3時間でセッションが回復するから、重いリファクタはそれからにするか」という判断が、
ウィンドウを切り替えず視線の移動だけでできます。

![作業中の画面に浮かぶHUD](docs/images/hud-in-action.png)

- 常に最前面の半透明パネル。**全Space・フルスクリーンアプリの上でも表示**、ドラッグで自由に配置(位置記憶)。
- 各行にリセットまでの残り時間(「あと3時間10分」)を常時表示(5時間枠が無いサービスは週次のリセットまで)。
- シェブロン(⌄)で展開すると**全ウィンドウの%とリセット日時**を一覧表示。
- 下部ボタン: ダッシュボードを開く / メニューバー表示切替 / 閉じる — **メニューバーを非表示にしてHUDだけで運用**も可能。
...
- **アクティビティ**: GitHub 風の日別ヒートマップを **Claude(オレンジ)/ Codex(モノクロ)の2枚上下に並べて同時表示**。色の濃さ = その日に消費した枠の量(%ポイント換算)。**アカウント全体の値ベースなので、他のマシンや Linux サーバでの利用も含まれます**。同じタブの下部にインサイト(上限到達回数・平均到達時間・平均週次使用率・高負荷日数を Claude / Codex 別に自動集計)。

![ダッシュボード アクティビティとインサイト](docs/images/dashboard-activity.png)
...
- ログイン時の自動起動はポップオーバーの「ログイン時に起動」をONにするだけです。
